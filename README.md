...
<!-- live-record:start -->
| 5-day regime-change forecaster | frozen test · 2019+ · scored once (n = 5,922) | **live forward record** · since 2026-08-17 · 24 resolved of 39 |
|---|---|---|
| PR-AUC ↑ | 0.548 (base rate 0.162) | — |
| Brier ↓ | 0.102 (base rate 0.136) | 0.062 (base rate 0.029) |
| precision · recall @ 0.22 | 0.45 · 0.59 | 0.00 · — |
| positive rate | 16% | 0% |

**Since 2026-08-17: live PR-AUC — / Brier 0.062 on 24 unseen forecasts, vs frozen test PR-AUC 0.548 / Brier 0.102.** Every weekday the pipeline appends its just-published forecasts (one row per pair, newest date only, never backfilled) to an append-only SHA-256 hash-chained ledger (`data/ledger.parquet`) *before* the outcome exists; five trading days later each row is resolved against the regimes that actually arrived and scored with the same code as the frozen test. A model refit starts a new segment — it cannot rewrite this one. Chain ✓ verified · updated 2026-09-03.
<!-- live-record:end -->
