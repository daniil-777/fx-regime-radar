"""Pairs — one market in depth. Reads small artifacts only (CLAUDE.md rule 8); no models here.

Close with filtered regime bands (and the orb — the one ambient element), the weather card of the
selected pair, out-of-sample regime anatomy, the anomaly siren. Universe switch and the scenario
explorer ("as of" date) live in the sidebar; every number is filtered/causal, so replaying history is
legitimate and nothing after the chosen date is drawn.
"""
...
st.markdown(
    f'<div class="fx-header"><div><span class="fx-wordmark">Pairs</span>'
    f'<span class="fx-sub">{html.escape(UNI.display(pair))} in depth · {html.escape(UNI.label)}</span></div>'
...
# --------------------------------------------------------------------------------------
# the selected pair's weather card (full detail)
# --------------------------------------------------------------------------------------
gp = regimes[regimes["pair"] == pair].sort_values("date")
if gp.empty:
    ui.state(
    ...
    st.stop()
latest = gp.iloc[-1]
if pair in api_latest:  # live state from the Rust service (same fields as the parquet row)
    latest = pd.Series(
        {**latest.to_dict(), **{k: v for k, v in api_latest[pair].items() if k in latest.index}}
    )
color = ui.REGIME_COLORS[latest["regime"]]
closes = prices[prices["pair"] == pair].sort_values("date")["close"].tail(20)
px_fmt = "{:.4f}" if closes.iloc[-1] < 100 else "{:,.0f}"
if time_machine:
    stats = narrate.build_stats(pair, regimes, None, prices)
    narration = {
    ...
    narration = report.get(pair)
card_left, card_right = st.columns([3, 2])
with card_left:
    ui.card(
        f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">'
        f'<span style="font-weight:500;font-size:1.05rem">{UNI.display(pair)}</span>{ui.regime_pill(latest["regime"], large=True)}</div>'
...
with card_right:
    ui.card(
        ui.narration(narration) or '<div class="fx-muted">No narration for this pair yet.</div>',
...
with title_col:
    st.markdown(
        ...
        f'<span style="font-weight:500">{UNI.display(pair)} — close with filtered regime bands{" · as of " + as_of.strftime("%Y-%m-%d") if time_machine else ""}</span><span>{legend}</span></div>',
        unsafe_allow_html=True,
    )
markers = load_json(str(DATA_DIR / "event_markers.json"), _mtime(DATA_DIR / "event_markers.json"))
fig = go.Figure()
for etype, symbol in (
    ("SNB", "diamond"),
    ("ECB", "circle"),
    ...
    dates = [pd.Timestamp(d) for d in (markers.get(etype) or [])]
    dates = [d for d in dates if len(g) and g["date"].min() <= d <= as_of]
    if dates:  # scheduled central-bank decisions: tiny markers on the baseline (phase 23)
        fig.add_trace(
            go.Scatter(
                x=dates,
                y=[0.05] * len(dates),
                ...
                mode="markers",
                name=etype,
                marker=dict(symbol=symbol, size=6, color=ui.DIM),
                hovertemplate=f"{etype} decision · %{{x|%Y-%m-%d}}<extra></extra>",
                showlegend=False,
            )
        )
fig.add_trace(
...
fig.update_layout(
    ...
    xaxis=dict(
        rangeselector=dict(
            ...
            font=dict(color=ui.TEXT, family=ui.FONT_UI),
        ),
    ),
    yaxis=dict(
        fixedrange=False,
